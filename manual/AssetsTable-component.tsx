// =============================================
// WAREHOUSE ASSETS TABLE COMPONENT
// =============================================
// File: apps/web/src/components/warehouse/AssetsTable.tsx
...
export function AssetsTable({ 
  ...
  const getStatusBadge = (status: string) => {
    const variants: Record<string, any> = {
      ...
      in_use: { variant: 'default', className: 'bg-blue-500' },
      maintenance: { variant: 'default', className: 'bg-yellow-500' },
      rented: { variant: 'default', className: 'bg-purple-500' },
    ...
    return (
      <Badge {...config}>
        {status === 'available' ? 'Saadaval' :
         status === 'in_use' ? 'Kasutuses' :
         status === 'maintenance' ? 'Hoolduses' :
         status === 'rented' ? 'Rendis' :
         status === 'retired' ? 'Välja arvatud' :
         status === 'lost' ? 'Kadunud' :
         status === 'damaged' ? 'Kahjustatud' :
         status}
      </Badge>
    );
  };
  ...
  const getConditionBadge = (condition: string) => {
    const variants: Record<string, any> = {
      excellent: { variant: 'default', className: 'bg-green-600' },
    ...
    return (
      <Badge {...config} className="text-xs">
        {condition === 'excellent' ? 'Suurepärane' :
         condition === 'good' ? 'Hea' :
         condition === 'fair' ? 'Rahuldav' :
         condition === 'poor' ? 'Halb' :
         condition}
      </Badge>
    );
          ...
          <TableHeader>
            <TableRow>
              <TableHead className="w-12">
                <input
                  type="checkbox"
                  checked={selectedAssets.length === data?.data.length}
              ...
              <TableHead>Järgmine hooldus</TableHead>
            ...
            {isLoading ? (
              ...
              data?.data.map((asset: Asset) => (
                ...
                >
                  ...
                  <TableCell>
                    {asset.photos?.[0]?.thumbnail_url ? (
                      <Image
                        src={asset.photos[0].thumbnail_url}
                        alt={asset.name}
                        width={40}
                        height={40}
